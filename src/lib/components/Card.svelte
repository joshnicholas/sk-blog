...
<svelte:element this={as} class={['relative flex flex-col items-start group', _class].join(' ')}>
  ...
  {#if $$slots.title}
    <div class="text-center justify-center text-xs  font-semibold text-zinc-800 dark:text-zinc-10">
      {#if href}
        <!-- <div
        ...
        <a {href} data-sveltekit-prefetch>
          <span class="absolute z-20 -inset-y-6 -inset-x-4 sm:-inset-x-6 sm:rounded-2xl text-zinc-800 dark:text-zinc-300" />
          <span class="relative z-10 text-zinc-800 dark:text-zinc-300">
            <slot name="title" />
          </span>
        </a>
      {:else}
  ...
  {#if $$slots.description}
    <div
      class="relative z-10 flex-1 text-sm text-zinc-800 dark:text-zinc-300"
